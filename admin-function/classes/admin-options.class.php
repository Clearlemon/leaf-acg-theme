    ...
    public function add_options_html()
    {
      ...
      echo '<div class="feng-admin">
        <div class="feng-banner">
                  <h2>叶主题设置页面</h2>
                  <h5 id="maple-madin-Banner-one-word"></h5>
        ...
        foreach ($this->pre_tabs as $tab) {
          ...
          if (!empty($tab['subs'])) {
            ...
            foreach ($tab['subs'] as $sub) {
              ...
              $sub_icon  = (!empty($sub['icon'])) ? '<svg class="icon" aria-hidden="true"><use class="maple-ico-admin" xlink:href="#' . esc_attr($sub['icon']) . '"></use></svg>' : '';
